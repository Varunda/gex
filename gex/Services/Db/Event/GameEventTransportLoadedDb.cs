...
namespace gex.Services.Db.Event {
    ...
    public class GameEventTransportLoadedDb : BaseGameEventDb<GameEventUnitTransportLoaded> {

        public GameEventTransportLoadedDb(ILoggerFactory loggerFactory, IDbHelper dbHelper)
            : base("game_event_unit_transport_loaded", "transport_loaded", loggerFactory, dbHelper) { }

        protected override void SetupInsert(GameEventUnitTransportLoaded ev, NpgsqlCommand cmd) {

            cmd.CommandText = @"
                INSERT INTO game_event_unit_transport_loaded (
                    game_id, frame,
