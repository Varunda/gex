...
namespace gex.Common.Services.Lobby.Implementations {
    ...
    public class LobbyClient : ILobbyClient {
        ...
        /// <summary>
        ///     connect to a spring lobby
        /// </summary>
        /// <param name="host">host. can be an ip or fqdn</param>
        /// <param name="port">port the host is running on</param>
        /// <param name="cancel"></param>
        /// <returns></returns>
        public async Task<Result<bool, string>> Connect(string host, int port, CancellationToken cancel) {
        ...
        /// <param name="cancel"></param>
        /// <returns></returns>
        public async Task<Result<LobbyMessage, string>> Login(string username, string password, CancellationToken cancel) {
            if (_LoggingIn == true) {
            ...
            _Logger.LogDebug($"logging in to Spring lobby [user={username}]");

            byte[] md5 = MD5.HashData(Encoding.UTF8.GetBytes(password));
            string hashedPass = Convert.ToBase64String(md5);

            Result<LobbyMessage, string> loginRequest = await WriteReply(
                "LOGIN", $"{username} {hashedPass} 0 * LuaLobby Chobby\t:3 :3\tb sp", TimeSpan.FromSeconds(5), cancel
            );
        ...
        public event EventHandler<LobbyMessage>? OnMessageReceived;
        public void EmitMessageReceived(LobbyMessage msg) {
