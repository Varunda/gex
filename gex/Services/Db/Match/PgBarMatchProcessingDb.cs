...
namespace gex.Services.Db.Match {
    ...
    public class PgBarMatchProcessingDb : IBarMatchProcessingDb {

        private readonly ILogger<PgBarMatchProcessingDb> _Logger;
        private readonly IDbHelper _DbHelper;

        public PgBarMatchProcessingDb(ILogger<PgBarMatchProcessingDb> logger,
            IDbHelper dbHelper) {
        ...
        public async Task Upsert(BarMatchProcessing proc) {
            if (string.IsNullOrEmpty(proc.GameID)) {
                throw new Exception($"missing GameID from bar match processing entry");
            }
        ...
        public async Task<List<BarMatchProcessing>> GetPriorityList(int count, CancellationToken cancel) {
            if (count > 1000) {
                throw new Exception($"failsafe, only allowing a max of 1000 for count, got {count}");
            }
