    ...
    public static class DbCommandExtensionMethod {
        ...
        public static void AddParameter(this DbCommand cmd, string name, object? value) {
            ...
            do {
                ...
                if (t == typeof(uint)) {
                    dbValue = unchecked((int)((uint)value));
                } else if (t == typeof(ulong)) {
                    dbValue = unchecked((long)((ulong)value));
                } else if (value is JsonElement json) {
                    if (json.ValueKind == JsonValueKind.Undefined) {
                        throw new ArgumentException($"cannot add an undefined JSON parameter [name={name}]");
                    }
                    ...
                    if (cmd is SqliteCommand) {
                        dbValue = json.ToString();
                    }
                } else if (cmd is SqliteCommand && value is DateTime dt) {
                    dbValue = (new DateTimeOffset(dt)).ToUnixTimeMilliseconds();
                }
        ...
        /// <returns></returns>
        public static string Print(this DbCommand cmd) {
            ...
            foreach (DbParameter param in cmd.Parameters) {
                ...
                if (valueType != null && valueType.IsGenericType && valueType.GetGenericTypeDefinition() == typeof(List<>)) {
                    ...
                    MethodInfo? boundPrint = typeof(DbCommandExtensionMethod).GetMethod("IterateUnboundGeneric")?.MakeGenericMethod(listType);
                    if (boundPrint == null) {
        ...
        /// </exception>
        public static async Task<int> ExecuteInt32(this DbCommand cmd, CancellationToken cancel) {
        ...
        /// </exception>
        public static async Task<long> ExecuteInt64(this DbCommand cmd, CancellationToken cancel) {
            object? objID = await cmd.ExecuteScalarAsync(cancel);
        ...
        ///     not be parsed to a valid long
        /// </exception>
        public static async Task<ulong> ExecuteUInt64(this DbCommand cmd, CancellationToken cancel) {
            object? objID = await cmd.ExecuteScalarAsync(cancel);
        ...
        /// </returns>
        public static async Task<T?> ExecuteReadSingle<T>(this DbCommand cmd, IDataReader<T> reader, CancellationToken cancel) where T : class {
        ...
        /// </returns>
        public static async Task<List<T>> ExecuteReadList<T>(this DbCommand cmd, IDataReader<T> reader, CancellationToken cancel) where T : class {
