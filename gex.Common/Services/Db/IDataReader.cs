...
namespace gex.Common.Services.Db {
    ...
    public abstract class IDataReader<T> where T : class {
        ...
        /// </returns>
        public abstract T? ReadEntry(DbDataReader reader);
        ...
        public Task<List<T>> ReadList(DbCommand cmd) {
        ...
        /// </returns>
        public async Task<List<T>> ReadList(DbCommand cmd, CancellationToken cancel) {
            List<T> entries = new List<T>();

            using DbDataReader reader = await cmd.ExecuteReaderAsync(cancel);
        ...
        /// </returns>
        public Task<T?> ReadSingle(DbCommand cmd) {
        ...
        ///     The value read from the command or null if no rows were returned
        /// </returns>
        public async Task<T?> ReadSingle(DbCommand cmd, CancellationToken cancel) {
            T? entry = null;
