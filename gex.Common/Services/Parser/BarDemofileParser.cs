...
using gex.Common.Models.Demofile;
...
namespace gex.Common.Services.Parser {
    ...
    public class BarDemofileParser {
        ...
        public BarDemofileParser(ILogger<BarDemofileParser> logger,
            LuaCommandParser commandParser, PolygonStartboxUtil polygonStartboxUtil,
            BarMapRepository? mapRepository = null,
            BarUserRepository? userRepository = null
        ) {
        ...
        private async Task<Result<BarMatch, string>> ReadBytes(string filename, byte[] data, DemofileParserOptions options, CancellationToken cancel) {
            ...
            header.GameID = Convert.ToHexStringLower(reader.Read(16).ToArray());
