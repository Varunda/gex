<template>
    <div>
        <h2 class="wt-header bg-primary">Map</h2>

        <div class="d-flex justify-content-lg-center mb-2 flex-wrap" style="gap: 0.5rem">
            <toggle-button v-model="map.startingBox"> Starting box </toggle-button>

            <toggle-button v-model="map.startingPosition"> Starting positions </toggle-button>

            <div class="btn-group border border-light">
                <button
                    type="button"
                    class="btn"
                    :class="[map.deathHeatmap ? 'btn-primary' : 'btn-dark']"
                    @click="map.deathHeatmap = !map.deathHeatmap"
                    :disabled="!hasEvents"
                >
                    Unit death heatmap
                </button>
            ...
            <toggle-button v-model="map.commanderPositions" :disabled="!hasEvents"> Com positions </toggle-button>

            <toggle-button v-model="map.commanderHeatmap" :disabled="!hasEvents"> Com heatmap </toggle-button>

            <toggle-button v-model="map.factories" :disabled="!hasEvents"> Factories </toggle-button>

            <toggle-button v-model="map.radars" :disabled="!hasEvents"> Radars </toggle-button>

            <toggle-button v-model="map.staticDefense" :disabled="!hasEvents"> Static defense </toggle-button>

            <toggle-button v-model="map.buildingHeatmap" :disabled="!hasEvents"> Building heatmap </toggle-button>
        </div>
        ...
        <div v-if="isMapImageLoading == true" :style="loadingRectangleDimensions" class="text-center">Loading map image...</div>

        <img id="map-dims" :src="mapUrl" style="display: none" />

        <div class="mt-2 mx-2">
            <button v-if="playback.playing == false" class="btn btn-sm btn-primary" @click="startUnitPositionPlayback" :disabled="!hasEvents">Play</button>

            <button v-else class="btn btn-sm btn-primary" @click="pauseUnitPositionPlayback" :disabled="!hasEvents">Pause</button>

            <span> Viewing units at {{ (playback.frame / 30) | mduration }} </span>

            <div class="mt-1">
                <input
                    type="range"
                    min="0"
                    :max="unitPositionFrames[unitPositionFrames.length - 1]"
                    step="900"
                    class="form-range"
                    v-model.number="playback.frame"
                    :disabled="!hasEvents"
                />
            </div>
        </div>
    ...
    export const MatchMap = Vue.extend({
        ...
        mounted: function (): void {
            ...
            this.$nextTick(() => {
                ...
                img.addEventListener("load", (ev: Event) => {
                    ...
                    ROOT = this.svg.append("g").attr("id", "doc-root");
                    ...
                    ROOT.append("image").classed("map-no-remove", true).attr("width", this.imgW).attr("height", this.imgH).attr("href", this.mapUrl);

                    ROOT.append("rect")
                        .classed("map-no-remove", true)
                        .attr("x", 0)
                        .attr("y", 0)
                        .attr("width", this.imgW)
                        .attr("height", this.imgH)
                        .style("fill", "#0a224244");
        ...
        methods: {
            ...
            drawMap: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot draw map: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot draw map: root is null`);
                }
            ...
            drawInitialUnitPositionFrame: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot render frame: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot render frame: root is null`);
                }
                ...
                const g = this.root.append("g").attr("id", "unit-pos-root");
                ...
                for (const pos of unitPos) {
                    ...
                    if (unitDef.speed == 0) {
                        g.append("rect")
                            ...
                            .attr("y", this.toImgZ(pos.z))
                            .attr("width", `${sizePx}px`)
                            .attr("height", `${sizePx}px`)
            ...
            addPlayerStartingPositions: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot add start pos: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot add start pos: root is null`);
                }
            ...
            addStartingBoxes: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot add starting box: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot add starting box: root is null`);
                }
                ...
                for (const allyTeam of this.match.allyTeams) {
                    ...
                    console.log(
                        `MatchMap> allyTeam ${allyTeam.allyTeamID} first player name ${firstPlayer?.username} is ${firstPlayer?.hexColor} at ${JSON.stringify(allyTeam.startBox)}`
                    );
            ...
            addCommanderPositionUpdates: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot add com pos: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot add com pos: root is null`);
                }
                ...
                for (const update of this.computedData.commander) {
                    ...
                    const color: string =
                        "#" +
                        (this.match.players
                            .find((iter) => iter.teamID == update.teamID)
                            ?.color.toString(16)
                            .padStart(6, "0") ?? `ffffff`);
                    let path: string = ``;

                    update.positions.sort((a, b) => {
            ...
            addUnitDeathHeatmap: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot add death heatmap: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot add death heatmap: root is null`);
                }

                // only include unit deaths from before a team is killed
                // otherwise the heatmap is just the units at the end blowing up lol
                const teamDiedAt: Map<number, number> = new Map(
                    this.output.teamDiedEvents.map((iter) => {
                        return [iter.teamID, iter.frame];
                    })
                );

                for (const player of this.match.players) {
                    const deathLocations: [number, number][] = this.output.unitsKilled
                        .filter((iter) => iter.teamID == player.teamID)
                        .filter((iter) => {
                    ...
                    this.root
                        ...
                        .attr("d", d3.geoPath())
                        .attr("fill", (d) => {
                            return `${player.hexColor}${Math.floor(lerp(d.value / max))
                                .toString(16)
                                .padStart(2, "0")}`;
                        });
                }
            ...
            addCommanderHeatmap: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot add com heatmap: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot add com heatmap: root is null`);
                }
            ...
            addBuildingHeatmap: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot add building heatmap: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot add building heatmap: root is null`);
                }
                ...
                for (const player of this.match.players) {
                    const locs: [number, number][] = this.output.unitsCreated
                        .filter((iter) => {
            ...
            addFactoryPositions: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot add factories: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot add factories: root is null`);
                }
            ...
            addRadars: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot add radars: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot add radars: root is null`);
                }
                ...
                for (const ev of this.output.unitsCreated) {
                    ...
                    this.createHoverRange(
                        `radar-range-${ev.unitID}`,
                        ev.unitID,
                        "map-radar",
                        "#00FF0044",
                        ev.unitX - unitDef.sizeX / 2,
                        ev.unitZ - unitDef.sizeZ / 2,
                        unitDef.radarDistance,
                        unitDef.sizeX,
                        unitDef.sizeZ,
                        player.hexColor,
                        player.color,
                        unitDef.definitionName
                    );
                }
            },
            ...
            addStaticDefense: function (): void {
                if (this.svg == null) {
                    return console.warn(`cannot add static defense: svg is null`);
                }
                if (this.root == null) {
                    return console.warn(`cannot add static defense: root is null`);
                }
                ...
                for (const ev of this.output.unitsCreated) {
                    ...
                    const g = this.createHoverRange(
                        `static-defense-${ev.unitID}`,
                        ev.unitID,
                        "map-static-defense",
                        "#FF000022",
                        ev.unitX - unitDef.sizeX / 2,
                        ...
                        unitDef.definitionName,
                        {
                            mouseenter: (ev: any) => {
                                ...
                                this.showTooltip(`${unitDef.name}<br>` + `Killed ${count} units`);
                            },
                            mousemove: (ev: any) => {
                                this.moveTooltip(ev);
                            },
            ...
            toImgX(x: number): number {
                return (x / this.mapW) * this.imgW;
            },
            toImgZ(z: number): number {
                return (z / this.mapH) * this.imgH;
            },
        ...
        watch: {
            ...
            "map.commanderPositions": function (): void {
                if (this.map.commanderPositions == false) {
                    this.root?.selectAll(".commander-updates").style("opacity", "0").style("pointer-events", "none");
                } else {
                    this.root?.selectAll(".commander-updates").style("opacity", "1").style("pointer-events", "auto");
                }
            },

            "map.staticDefense": function (): void {
                if (this.map.staticDefense == false) {
                    this.root?.selectAll(".map-static-defense").style("opacity", "0").style("pointer-events", "none");
                } else {
                    this.root?.selectAll(".map-static-defense").style("opacity", "1").style("pointer-events", "auto");
                }
            },

            "map.radars": function (): void {
                if (this.map.radars == false) {
                    this.root?.selectAll(".map-radar").style("opacity", "0").style("pointer-events", "none");
                } else {
                    this.root?.selectAll(".map-radar").style("opacity", "1").style("pointer-events", "auto");
                }
            },

            "map.factories": function (): void {
                if (this.map.factories == false) {
                    this.root?.selectAll(".map-factory").style("opacity", "0").style("pointer-events", "none");
                } else {
                    this.root?.selectAll(".map-factory").style("opacity", "1").style("pointer-events", "auto");
                }
            },

            "map.startingBox": function (): void {
                if (this.map.startingBox == false) {
                    this.root?.selectAll(".map-starting-box").style("opacity", "0").style("pointer-events", "none");
                } else {
                    this.root?.selectAll(".map-starting-box").style("opacity", "1").style("pointer-events", "auto");
                }
            },
            ...
            "map.deathHeatmap": function (): void {
                if (this.map.deathHeatmap == false) {
                    this.root?.selectAll(".map-unit-death-heatmap").style("opacity", "0").style("pointer-events", "none");
                } else {
                    this.root?.selectAll(".map-unit-death-heatmap").style("opacity", "1").style("pointer-events", "auto");
                }
            },

            "map.commanderHeatmap": function (): void {
                if (this.map.commanderHeatmap == false) {
                    this.root?.selectAll(".map-commander-position-heatmap").style("opacity", "0").style("pointer-events", "none");
                } else {
                    this.root?.selectAll(".map-commander-position-heatmap").style("opacity", "1").style("pointer-events", "auto");
                }
            },

            "map.buildingHeatmap": function (): void {
                if (this.map.buildingHeatmap == false) {
                    this.root?.selectAll(".map-building-heatmap").style("opacity", "0").style("pointer-events", "none");
                } else {
                    this.root?.selectAll(".map-building-heatmap").style("opacity", "1").style("pointer-events", "auto");
                }
            },

            "map.startingPosition": function (): void {
                if (this.map.startingPosition == false) {
                    this.root?.selectAll(".map-starting-position").style("opacity", "0").style("pointer-events", "none");
                } else {
                    this.root?.selectAll(".map-starting-position").style("opacity", "1").style("pointer-events", "auto");
                }
            },
        },
