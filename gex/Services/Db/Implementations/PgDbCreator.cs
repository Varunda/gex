...
namespace gex.Services.Db.Implementations {
    ...
    public class PgDbCreator : IDbCreator {

        private readonly ILogger<PgDbCreator> _Logger;
        private readonly IDbHelper _DbHelper;
        private readonly IOptions<InstanceOptions> _InstanceOptions;

        private readonly bool _RunDb = true;

        public PgDbCreator(ILogger<PgDbCreator> logger,
        ...
        public async Task Execute() {
            ...
            using DbCommand cmd =  await _DbHelper.Command(conn, @"CREATE EXTENSION IF NOT EXISTS pg_trgm;");
            ...
            if (_InstanceOptions.Value.SplitDatabases == true) {
                ...
                using DbCommand evCmd = await _DbHelper.Command(evConn, @"CREATE EXTENSION IF NOT EXISTS postgres_fdw;");
            }
