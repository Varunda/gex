...
namespace gex.Common.Services.Repository.Match {

    public class BarMatchTeamRepository {

        private readonly ILogger<BarMatchTeamRepository> _Logger;
        private readonly IBarMatchTeamDb _Db;
        private readonly IMemoryCache _Cache;
        
        private const string CACHE_KEY_ID = "Gex.MatchTeams.{0}"; // {0} => game ID
        private const string CACHE_KEY_UNIQUE_COLORS = "Gex.MatchTeams.UniqueColors"; // game ID cannot contain these characters, no collision

        private static readonly SemaphoreSlim _Lock = new(1, 1);

        public BarMatchTeamRepository(ILogger<BarMatchTeamRepository> logger,
            IBarMatchTeamDb db, IMemoryCache cache) {
        ...
        public async Task<List<BarMatchTeam>> GetByGameID(string gameID, CancellationToken cancel) {
            ...
            if (_Cache.TryGetValue(cacheKey, out List<BarMatchTeam>? players) == false || players == null) {
        ...
        public async Task<List<BarMatchTeam>> GetByGameIDs(IEnumerable<string> IDs, CancellationToken cancel) {
            ...
            foreach (string ID in localIDs.ToList()) {
                ...
                if (_Cache.TryGetValue(cacheKey, out List<BarMatchTeam>? matchTeams) == true && matchTeams != null) {
                    localIDs.Remove(ID);
                    t.AddRange(matchTeams);
                }
            }

            if (localIDs.Count > 0) {
                List<BarMatchTeam> dbMatches = await _Db.GetByGameIDs(localIDs, cancel);
                t.AddRange(dbMatches);
            }

            IEnumerable<IGrouping<string, BarMatchTeam>> teams = t.GroupBy(iter => iter.GameID);
            foreach (IGrouping<string, BarMatchTeam> groupedTeams in teams) {
                string cacheKey = string.Format(CACHE_KEY_ID, groupedTeams.Key);
                _Cache.Set(cacheKey, groupedTeams.ToList(), new MemoryCacheEntryOptions() {
                    AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(5)
                });
            }
        ...
        public async Task Insert(BarMatchTeam team, CancellationToken cancel) {
            string cacheKey = string.Format(CACHE_KEY_ID, team.GameID);
            _Cache.Remove(cacheKey);
            await _Db.Insert(team, cancel);
        }
